...
public class DevMenuTurboPatch
{
    ...
    [HarmonyPatch(typeof(UI.UIManager), "Update")]
    [HarmonyTranspiler]
    static IEnumerable<CodeInstruction> FixUnsafeUpgradePanelCast(IEnumerable<CodeInstruction> instructions)
    {
        foreach (var instruction in instructions)
        {
            if (instruction.opcode == OpCodes.Castclass && instruction.operand as Type == typeof(UI.Upgrades.Panel))
            {
